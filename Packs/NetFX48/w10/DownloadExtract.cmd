...
if not exist "%~2" (
	rem @start "正在下载 %~4 文件……" /wait /B PowerShell.exe -Command Invoke-WebRequest "%~1" -OutFile "%~2"
	rem @start "正在下载 %~4 文件……" /wait /B PowerShell.exe -Command "(New-Object System.Net.WebClient).DownloadFile('%~1', '%~2')"
	@start "正在下载 %~4 文件……" /wait /B PowerShell.exe -Command Start-BitsTransfer -Source "%~1" -Destination "%~2"
	rem @start "正在下载 %~4 文件……" /wait /B PowerShell.exe -Command Start-BitsTransfer -Source %~3\*.txt -Destination %~3\ -TransferType Download
) else (
	echo.
	echo %~4 文件已存在。
